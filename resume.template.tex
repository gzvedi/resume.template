...
% \item [$if(sections.items.label)$$for(sections.items.label)$$sections.items.label$$sep$ \newline $endfor$$endif$] $for(sections.items.text)$$sections.items.text$$sep$ \newline $endfor$
$for(sections.items)$
\item [ $if(sections.items.label)$$sections.items.label$$endif$ ]
  $for(sections.items.text)$
    $sections.items.text$$sep$ \newline
  $endfor$
$endfor$
